...
def data_setup(): # From Dr. Ning
    ...
    batch = int(64 * (4/n)**2)
    train_loader = DataLoader([Data(x=X_train[i], y=y_train[i], edge_index=edge_index) for i in range(len(X_train))], batch_size=batch, shuffle=True) 
...
class GN(MessagePassing):
    ...
    def message(self, x_i, x_j):
        ...
        tmp = torch.cat([x_i, x_j], dim=1) #shape is [n_edges, 2*n_features] 
...
model.train()
training_losses = []
saver = 100 #save every 100 epochs
for epoch in range(1):
    ...
    # save loss every saver 
    if epoch % saver == 0:
        torch.save(model, f'model_{epoch}_epochs')
...
torch.save(model, 'model_800_epochs')
model.eval()
...
# %% Just in case, load and plot on testing all models
model100 = torch.load('model_100_epochs')
model200 = torch.load('model_200_epochs')
model300 = torch.load('model_300_epochs')
model400 = torch.load('model_400_epochs')
model500 = torch.load('model_500_epochs')
model600 = torch.load('model_600_epochs')
model700 = torch.load('model_700_epochs')
model800 = model
model100.eval()
model200.eval()
model300.eval()
...
model = model300
my_trajs_300 = solve_ivp(ode_func, (times[0], times[-1]), x0_test.flatten(), t_eval=times).y.reshape(4,5,len(times))
model = model500
my_trajs_500 = solve_ivp(ode_func, (times[0], times[-1]), x0_test.flatten(), t_eval=times).y.reshape(4,5,len(times))
model = model600
my_trajs_600 = solve_ivp(ode_func, (times[0], times[-1]), x0_test.flatten(), t_eval=times).y.reshape(4,5,len(times))
model = model700
my_trajs_700 = solve_ivp(ode_func, (times[0], times[-1]), x0_test.flatten(), t_eval=times).y.reshape(4,5,len(times))
...
plt.figure()
for j in range(4):
    plt.plot(my_trajs_300[j,0,:], my_trajs_300[j,1,:])
    plt.plot(test_traj[:, j, 0], test_traj[:, j, 1], '--')
plt.xlabel('x position')
plt.ylabel('y position')
plt.title('Testing Predicted Trajectories (300)')
plt.show()

plt.figure()
for j in range(4):
    plt.plot(my_trajs_500[j,0,:], my_trajs_500[j,1,:])
    plt.plot(test_traj[:, j, 0], test_traj[:, j, 1], '--')
plt.xlabel('x position')
plt.ylabel('y position')
plt.title('Testing Predicted Trajectories (500)')
plt.show()

plt.figure()
for j in range(4):
    plt.plot(my_trajs_600[j,0,:], my_trajs_600[j,1,:])
    plt.plot(test_traj[:, j, 0], test_traj[:, j, 1], '--')
plt.xlabel('x position')
plt.ylabel('y position')
plt.title('Testing Predicted Trajectories (600)')
plt.show()

plt.figure()
for j in range(4):
    plt.plot(my_trajs_700[j,0,:], my_trajs_700[j,1,:])
    plt.plot(test_traj[:, j, 0], test_traj[:, j, 1], '--')
plt.xlabel('x position')
plt.ylabel('y position')
plt.title('Testing Predicted Trajectories (700)')
plt.show()

# %% model500 performed the best, so I will put a vertical line on the loss plot 
# for the loss plot I need the testing loss from each model and will scatter it on
#Get testing loss
all_testing_losses = []
def test_loss_grabber(modelf):
    modelf.eval()
    testing_losses = []
    for batch in testing_loader:
        batch = batch.to(device)
        testing_losses.append(Losser(modelf, batch).item())
    return np.mean(testing_losses)
all_testing_losses.append(test_loss_grabber(model100))
all_testing_losses.append(test_loss_grabber(model200))
all_testing_losses.append(test_loss_grabber(model300))
all_testing_losses.append(test_loss_grabber(model400))
all_testing_losses.append(test_loss_grabber(model500))
all_testing_losses.append(test_loss_grabber(model600))
all_testing_losses.append(test_loss_grabber(model700))
all_testing_losses.append(test_loss_grabber(model800))

#%% Training and testing loss plot 
plt.figure()
plt.semilogy(training_losses, label='Training Loss')
plt.scatter([100,200,300,400,500,600,700,800], all_testing_losses, color='orange', label='Testing Loss')
plt.scatter(500, all_testing_losses[4], edgecolor='red', facecolors='none', s=80, lw = 2, label='Best (chosen) Model')
plt.xlabel('Epochs')
plt.ylabel('Loss')
...
plt.figure()
for j in range(4):
    plt.plot(my_trajs_500[j,0,:], my_trajs_500[j,1,:], '--', color=colors[j], label=f'Predicted particle {j} ')
    plt.plot(test_traj[:, j, 0], test_traj[:, j, 1],  color=colors[j], label=f'True particle {j}')
plt.xlabel('x position')
plt.ylabel('y position')
plt.title('Testing Predicted Trajectories (model from 500 epochs)')
