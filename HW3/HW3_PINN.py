...
def Burgers(model, collptsf): # collocation points (ncol, 1)
    ...
    u = model(collptsf)
    # compute derivatives
    u_all = torch.autograd.grad(u, collptsf, grad_outputs = torch.ones_like(u), create_graph=True)[0] 
...
def conditions(model,condptsf): #boundary and initial conditions (nbc_locations,1)
    ...
    ic_loss = uc[t0pts] + torch.sin(torch.pi * condptsf[t0pts,1].reshape(50,1))
...
for e in range(epochs): 
    ...
    train_loss = total_loss(model, condition_points_tensor, collocation_points_tensor, 1., 0, 0, 0.) #I had 1,10,10,100000: ic, bc (neg), bc(pos), data/physics
    train_loss.backward()
    optimizer.step()
